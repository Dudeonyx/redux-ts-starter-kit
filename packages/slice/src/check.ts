...
type Test = {
  ...
  hf: (state: number, action: PayloadAction<number, 'hi'>) => number;
  //   [s:string]: any
};
...
const R4 = test({
  cases: {
    ...
    h3: (state, action: PayloadAction) => 5,
  },
  state: 5,
});
const R5 = test({
 ...
 */
export type CaseReducers<S, AS> = {
  [T in keyof AS]: AS[T] extends PayloadAction<any, any>
    ? CaseReducer<S, AS[T]>
    : CaseReducer<S, PayloadAction<undefined>>
};
...
function test<S, CR extends CaseReducers<S, ActionsMap>>({
  cases,
}: {
  cases: CR;
  state: S;
